...
function PulseBlock({ className }: { className?: string }) {
  return <div className={cn('animate-pulse rounded-[1.35rem] border-2 border-black bg-[#f8c8ed]/70 shadow-[4px_4px_0_#171717]', className)} />
}
...
export function PageLoadingState({ label = 'Loading page', className }: LoadingStateProps) {
  return (
    <div className={cn('mx-auto w-full max-w-7xl px-4 py-12 sm:px-6 lg:px-8', className)} aria-live="polite" aria-busy="true">
      <p className="w-fit rounded-full border-2 border-black bg-[#ffd23f] px-4 py-2 text-xs font-black uppercase tracking-[0.24em] text-black">{label}</p>
      <PulseBlock className="mt-5 h-12 w-3/4 max-w-3xl" />
      <PulseBlock className="mt-4 h-5 w-2/3 max-w-2xl" />
      <div className="mt-8 grid gap-4 md:grid-cols-3">
        {[0, 1, 2].map((item) => (
          <div key={item} className="rounded-[1.5rem] border-2 border-black bg-[#fffdf6] p-5 shadow-[6px_6px_0_#171717]">
  ...
  return (
    <div className={cn('grid gap-5 sm:grid-cols-2 lg:grid-cols-3', className)} aria-live="polite" aria-busy="true">
      {Array.from({ length: count }).map((_, index) => (
        <div key={index} className="rounded-[1.5rem] border-2 border-black bg-[#fffdf6] p-4 shadow-[5px_5px_0_#171717]">
