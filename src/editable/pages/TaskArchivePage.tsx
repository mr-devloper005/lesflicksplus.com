...
const cardBase = 'group block rounded-[var(--tk-radius)] border-2 border-[var(--tk-line)] bg-[var(--tk-surface)] shadow-[6px_6px_0_var(--tk-line)] transition duration-500 hover:-translate-y-1.5 hover:translate-x-0.5 hover:shadow-[10px_10px_0_var(--tk-accent)]'

const archiveTopAdSlot = (task: TaskKey) => task === 'listing' ? 'in-feed' : 'header'
const archiveBottomAdSlot = (task: TaskKey) => task === 'listing' ? 'footer' : 'article-bottom'
  ...
  return (
    <EditableSiteShell>
      <main style={taskThemeStyle(task)} className="min-h-screen bg-[var(--tk-bg)] text-[var(--tk-text)]">
        <header className="relative overflow-hidden border-b-2 border-[var(--tk-line)] bg-[var(--tk-bg)]">
          <div className="pointer-events-none absolute -left-20 top-16 h-44 w-44 rounded-full border-2 border-black bg-[var(--tk-accent)] opacity-80 editable-float" />
          <div className="pointer-events-none absolute right-8 top-24 h-28 w-28 rotate-12 rounded-[2rem] border-2 border-black bg-[#ffd23f] opacity-90 editable-wiggle" />
          ...
          <div className="relative mx-auto max-w-[var(--editable-container)] px-6 py-14 sm:py-20 lg:px-8">
            {task !== 'article' ? (
              <div className="flex w-fit items-center gap-3 rounded-full border-2 border-black bg-white px-4 py-2 text-[11px] font-black uppercase tracking-[0.28em] text-[var(--tk-text)]">
                <span>{theme.kicker}</span>
                <span className="h-2 w-2 rounded-full bg-[var(--tk-accent)]" />
                <span>{label}</span>
              </div>
            ) : null}
            <h1 className="editable-display mt-6 max-w-4xl text-balance text-[2.5rem] leading-[0.98] tracking-[-0.065em] sm:text-5xl lg:text-6xl">
              {voice?.headline || `Browse ${label}`}
            </h1>
            <p className="mt-6 max-w-2xl text-lg font-semibold leading-8 text-[var(--tk-muted)]">{voice?.description || theme.note}</p>
            {voice?.chips?.length ? (
              <div className="mt-8 flex flex-wrap gap-2.5">
                {voice.chips.map((chip) => (
                  <span key={chip} className="rounded-full border-2 border-[var(--tk-line)] bg-[var(--tk-surface)] px-3.5 py-1.5 text-xs font-black text-[var(--tk-text)] transition hover:-translate-y-0.5 hover:bg-[var(--tk-accent-soft)]">{chip}</span>
                ))}
              </div>
            ) : null}

            <div className="mt-12 flex flex-col gap-4 border-t-2 border-[var(--tk-line)] pt-6 sm:flex-row sm:items-center sm:justify-between">
              <p className="rounded-full border-2 border-black bg-[#ffd23f] px-4 py-2 text-sm font-black text-black">
              ...
              <form action={basePath} className="flex items-center gap-2.5">
                <div className="relative">
                  <select
                    ...
                    className="h-12 appearance-none rounded-full border-2 border-[var(--tk-line)] bg-[var(--tk-surface)] pl-5 pr-10 text-sm font-black text-[var(--tk-text)] outline-none transition focus:bg-white focus:shadow-[4px_4px_0_var(--tk-accent)]"
                ...
                <button className="inline-flex h-12 items-center rounded-full border-2 border-black bg-black px-6 text-sm font-black text-white transition hover:bg-[var(--tk-accent)] hover:text-black">Apply</button>
              </form>
            </div>
          </div>
        </header>

        <ArchiveAd slot={archiveTopAdSlot(task)} eager />
        ...
        <section className="mx-auto max-w-[var(--editable-container)] px-6 py-16 sm:py-20 lg:px-8">
          {posts.length ? (
            ...
            <div className="mx-auto max-w-xl rounded-[var(--tk-radius)] border-2 border-dashed border-[var(--tk-line)] bg-[var(--tk-surface)] px-8 py-16 text-center shadow-[6px_6px_0_var(--tk-line)]">
          ...
          {posts.length ? (
            <nav className="mt-16 flex items-center justify-center gap-3 text-sm">
              {pagination.hasPrevPage ? <Link href={pageHref(basePath, category, page - 1)} className="rounded-full border-2 border-[var(--tk-line)] bg-white px-5 py-2.5 font-black transition hover:bg-[var(--tk-accent-soft)]">Previous</Link> : null}
              <span className="rounded-full border-2 border-[var(--tk-line)] bg-[#ffd23f] px-5 py-2.5 font-black text-black">Page {page} of {pagination.totalPages || 1}</span>
              {pagination.hasNextPage ? <Link href={pageHref(basePath, category, page + 1)} className="rounded-full border-2 border-[var(--tk-line)] bg-white px-5 py-2.5 font-black transition hover:bg-[var(--tk-accent-soft)]">Next</Link> : null}
            </nav>
          ) : null}
        </section>

        <ArchiveAd slot={archiveBottomAdSlot(task)} />
      </main>
    </EditableSiteShell>
  )
}
...
function ListingArchiveCard({ post, href }: { post: SitePost; href: string }) {
  ...
  return (
    <Link href={href} className={`${cardBase} flex items-center gap-5 p-5 sm:p-6`}>
      <div className="flex h-24 w-24 shrink-0 items-center justify-center overflow-hidden rounded-[1rem] border-2 border-[var(--tk-line)] bg-[var(--tk-raised)] shadow-[3px_3px_0_var(--tk-line)]">
...
function ImageArchiveCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  const image = getImage(post)
  return (
    <Link href={href} className="group mb-5 block break-inside-avoid overflow-hidden rounded-[var(--tk-radius)] border-2 border-[var(--tk-line)] bg-[var(--tk-surface)] shadow-[5px_5px_0_var(--tk-line)] transition duration-300 hover:-translate-y-1 hover:shadow-[8px_8px_0_var(--tk-accent)]">
